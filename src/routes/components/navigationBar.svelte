...
<nav>
    ...
    <a class="x" href="/" aria-label={$dictionary.inicio}>
        <img class="logo" src="/images/ccs/CCSlogoTransparent.webp" alt="CCS Logo">
    </a>
    
    <section id="xnav" class="x">
        <a href="/" class="link {$page.url.pathname === '/' ? 'active' : ''}">{$dictionary.inicio}</a>
        <a href="/acerca" class="link {$page.url.pathname === '/acerca' ? 'active' : ''}">{$dictionary.acercaDeNosotros}</a>
        <a href="/contactanos" class="link {$page.url.pathname === '/contactanos' ? 'active' : ''}">{$dictionary.contactanos}</a>
